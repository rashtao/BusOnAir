...
//    }
    ...
    public void add(boa.server.routing.Direction dir) {
        ...
        output.getWalks().addFirst(new DirectionWalk(
                ...
                new boa.server.plugin.json.Coordinate(s_arrivo.getLatitude(), s_arrivo.getLongitude()),
                new boa.server.plugin.json.Coordinate(dir.getLat(), dir.getLon()),
                s_arrivo.getId()));
            ...
            if (prevtmp != null) {
                ...
                if (!depStat.equals(tmp.getStation())) {        // è un cambio
                    output.getWalks().addFirst(new DirectionWalk(
                            ...
                            0,
                            new boa.server.plugin.json.Coordinate(tmp.getStation().getLatitude(), tmp.getStation().getLongitude()),
                            new boa.server.plugin.json.Coordinate(tmp.getStation().getLatitude(), tmp.getStation().getLongitude()),
                            tmp.getStation().getId()));
                } else {        // è la prima walk
                    Coordinate coord1 = new boa.server.plugin.json.Coordinate(lat1, lon1);
                    Coordinate coord2 = new boa.server.plugin.json.Coordinate(prevtmp.getStation().getLatitude(), prevtmp.getStation().getLongitude());
